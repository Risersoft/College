    ...
    if (contnetUnit == "true") {
        if (valMsg == "success") {
            if (fstatus == 'fileStatus') {
                ...
                $('#spnAMsg').html("<i class='fa fa-check-circle fa-2x' style='color: green'></i>  Your file has been uploaded successfully, if you want to upload another file so please continue with same process.");
                $scope.isAShowMsg = true;
            ...
            else if (fstatus == 'iqacStatus') {
                if (Array.isArray($scope.IQAC) && $scope.IQAC.length > 0) {
                    $scope.IQAC = [];
                }
                $scope.IQAC.push({ bloborgname: $('#iqac_attach_filename').val().split('\\')[2], blobname: flName.split("==")[0], filesize: (flName.split("==")[1] / 1024), lastmodified: new Date(flName.split("==")[2] * 1000).toGMTString(), blobUrl: flName.split("==")[4], UploadType: flName.split("==")[5], Url: $('#iqac_attach_filename').val().split('\\')[2] });
                console.log($scope.IQAC);
                $('#iqac_attach_filename').val('');
            }
            else if (fstatus == 'aqarStatus') {
                if (Array.isArray($scope.AQAR) && $scope.AQAR.length > 0) {
                    $scope.AQAR = [];
                }
                $scope.AQAR.push({ bloborgname: $('#aqar_attach_filename').val().split('\\')[2], blobname: flName.split("==")[0], filesize: (flName.split("==")[1] / 1024), lastmodified: new Date(flName.split("==")[2] * 1000).toGMTString(), blobUrl: flName.split("==")[4], UploadType: flName.split("==")[5], Url: $('#aqar_attach_filename').val().split('\\')[2] });
                console.log($scope.AQAR);
                $('#aqar_attach_filename').val('');
            }
            else if (fstatus == 'ssrStatus') {
                if (Array.isArray($scope.SSR) && $scope.SSR.length > 0) {
                    $scope.SSR = [];
                }
                $scope.SSR.push({ bloborgname: $('#ssr_attach_filename').val().split('\\')[2], blobname: flName.split("==")[0], filesize: (flName.split("==")[1] / 1024), lastmodified: new Date(flName.split("==")[2] * 1000).toGMTString(), blobUrl: flName.split("==")[4], UploadType: flName.split("==")[5], Url: $('#ssr_attach_filename').val().split('\\')[2] });
                console.log($scope.SSR);
                $('#ssr_attach_filename').val('');
            }
            else if (fstatus == 'agendaminutesStatus') {
                if (Array.isArray($scope.AgendaMinutes) && $scope.AgendaMinutes.length > 0) {
                    $scope.AgendaMinutes = [];
                }
                $scope.AgendaMinutes.push({ bloborgname: $('#agendaminutes_attach_filename').val().split('\\')[2], blobname: flName.split("==")[0], filesize: (flName.split("==")[1] / 1024), lastmodified: new Date(flName.split("==")[2] * 1000).toGMTString(), blobUrl: flName.split("==")[4], UploadType: flName.split("==")[5], Url: $('#agendaminutes_attach_filename').val().split('\\')[2] });
                console.log($scope.AgendaMinutes);
                $('#agendaminutes_attach_filename').val('');
            }
